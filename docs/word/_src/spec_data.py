...
FR = [
    ...
    ("FR-22", "Large, screen-relative controls",
     "Home action tiles shall share the height left on screen (minimum 140 dp phone / 200 dp "
     "tablet); primary and tonal buttons shall be at least 60 dp tall (72 dp on tablets); icon "
     "buttons at least 56 dp (64 dp); every touch target at least 48 dp.",
     "Must", ["SCR-02", "SCR-03", "SCR-04", "SCR-05", "SCR-07", "SCR-10"],
     ["dimens.xml", "values-sw600dp/dimens.xml", "themes.xml"]),
    ("FR-23", "Collapsible side menu",
     "The home side menu shall be collapsed (icons only, 80 dp / 104 dp) by default and "
     "expand to 208 dp / 280 dp with labels when the user presses the right-hand button of "
...
NFR = [
    ...
    ("NFR-07", "Accessibility",
     ...
     "shall have tooltips; touch targets shall be at least 48 dp.",
...
SCREENS = [
    ...
    dict(id="SCR-02", name="Home", activity="MainActivity", layout="activity_main.xml",
         ...
         purpose="Entry point: large action tiles, language pair, manual translation, side menu.",
         ...
         components=[
             (1, "Side-menu toggle (right-hand)", "ImageButton", "main_nav_toggle", "56 dp / 64 dp",
             ...
             (2, "Side menu", "LinearLayout of item_nav", "nav_rail, nav_home … nav_settings",
              "80 dp collapsed / 208 dp expanded (104 / 280 on tablets); items 64 dp (80 dp) tall",
             ...
             (4, "Action tiles (4)", "item_action_tile", "tile_image, tile_pdf, tile_sequence, tile_history",
              "Share remaining height, min 140 dp (200 dp); icon 44 dp (64 dp); title 20 sp (28 sp)",
              "Image OCR -> SCR-03; PDF OCR -> SCR-07; Image Sequence -> SCR-04; History -> SCR-08."),
             (5, "Language bar", "view_language_bar", "language_bar",
              "Swap button 48 dp (56 dp); names 17 sp", "Tap a side -> SCR-06 for that side; swap exchanges source and target."),
             (6, "Translation button", "MaterialButton (Outlined)", "main_manual_translate",
              "height 60 dp (72 dp), 18 sp (22 sp)", "Opens SCR-10."),
             (7, "Model status", "ImageButton", "main_models_status", "56 dp / 64 dp", "Opens SCR-09."),
    ...
    dict(id="SCR-03", name="Camera OCR", activity="CameraActivity", layout="activity_camera.xml",
         ...
         components=[
             (1, "Back", "ImageButton", "camera_back", "56 dp / 64 dp", "Closes the screen."),
             (2, "Flash", "ImageButton", "camera_flash", "56 dp / 64 dp", "Toggles flash mode (dimmed when off)."),
             (3, "Gallery (top)", "ImageButton", "camera_gallery_top", "56 dp / 64 dp", "Opens the system image picker."),
             (4, "Preview and frame", "PreviewView", "camera_preview", "Full screen", "Back camera preview; framing guide."),
             (5, "Gallery", "circle button", "camera_gallery", "56 dp / 64 dp", "Opens the system image picker."),
             (6, "Shutter", "ImageButton", "camera_shutter", "88 dp / 108 dp", "Captures and processes; ignored while busy."),
             (7, "Auto", "circle button", "camera_auto", "56 dp / 64 dp", "Shows 'Auto' (informational)."),
         ]),
    dict(id="SCR-04", name="Image Import", activity="ImageImportActivity", layout="activity_image_import.xml",
         ...
         components=[
             (1, "Toolbar", "view_toolbar", "toolbar_back", "64 dp bar, 56 dp buttons", "Back closes."),
             (2, "Thumbnail grid", "RecyclerView (3 columns)", "import_grid", "Square cells",
              "Tap toggles selection (red frame and check)."),
             (3, "Select images", "MaterialButton (Tonal)", "import_add", "60 dp / 72 dp", "Opens the multi-image picker."),
             (4, "Selection count", "TextView", "import_count", "12 sp", "'n images selected'."),
             (5, "Next", "MaterialButton (Primary)", "import_next", "60 dp / 72 dp",
    ...
    dict(id="SCR-05", name="Translation Result", activity="ResultActivity", layout="activity_result.xml",
         ...
         components=[
             (1, "Toolbar", "view_toolbar", "toolbar_back", "64 dp bar", "Back closes and releases page bitmaps."),
             ...
             (6, "Speak", "MaterialButton (Tonal)", "result_speak", "60 dp / 72 dp", "Reads the translation; disabled without a voice."),
             (7, "Show original / translated", "MaterialButton (Tonal)", "result_toggle", "60 dp / 72 dp", "Toggles the image."),
             (8, "Save", "MaterialButton (Primary)", "result_save", "60 dp / 72 dp", "Saves to history; toast 'Saved to history'."),
         ]),
    ...
    dict(id="SCR-07", name="PDF OCR", activity="PdfActivity", layout="activity_pdf.xml",
         ...
         components=[
             ...
             (2, "Clear", "ImageButton", "pdf_file_clear", "48 dp / 56 dp", "Clears the selected file."),
             (3, "Page Range", "item_option_row", "pdf_row_range", "min 64 dp", "All Pages, 1-5, 1-10, 1-20."),
             (4, "Output Format", "item_option_row", "pdf_row_format", "min 64 dp", "PDF, DOCX, TXT (persisted)."),
             (5, "Language bar", "view_language_bar", "pdf_language_bar", "-", "As on SCR-02."),
             (6, "Start OCR", "MaterialButton (Primary)", "pdf_start", "60 dp / 72 dp",
    ...
    dict(id="SCR-08", name="History", activity="HistoryActivity", layout="activity_history.xml",
         ...
         exit="Back.",
         components=[
             (1, "Clear history", "ImageButton (toolbar action)", "toolbar_action", "56 dp / 64 dp",
    ...
    dict(id="SCR-09", name="Settings", activity="SettingsActivity", layout="activity_settings.xml",
         ...
         exit="Back.",
         components=[
             (1, "OCR Language", "item_option_row", "settings_ocr_language", "min 64 dp", "Toggles Auto / configured source."),
             (2, "Image Quality", "item_option_row", "settings_image_quality", "min 64 dp", "LOW 640 / MEDIUM 960 / HIGH 1280 px."),
             (3, "Auto Translation", "item_option_row (switch)", "settings_auto_translate", "min 64 dp", "Translate after recognition."),
             (4, "Default Source Language", "item_option_row", "settings_default_source", "min 64 dp", "Opens SCR-06 (source)."),
             (5, "Default Target Language", "item_option_row", "settings_default_target", "min 64 dp", "Opens SCR-06 (target)."),
             (6, "Installed Models", "item_option_row", "settings_models", "min 64 dp",
              "'OCR languages / MT pairs' (3 / 4 with Korean hidden); tap shows report and pivoted pairs."),
             (7, "About", "item_option_row", "settings_about", "min 64 dp", "Version and model licences."),
             (8, "Help & Feedback", "item_option_row", "settings_help", "min 64 dp", "Usage tips."),
         ]),
    dict(id="SCR-10", name="Translation (manual)", activity="TranslateActivity", layout="activity_translate.xml",
         ...
         components=[
             ...
             (3, "Copy input / counter", "ImageButton, TextView", "translate_copy_source, translate_input_counter",
              "48 dp / 56 dp", "Copies input; shows n/500."),
             (4, "Translate", "MaterialButton (Primary)", "translate_action", "60 dp / 72 dp",
             ...
             (6, "Copy output", "ImageButton", "translate_copy_output", "48 dp / 56 dp", "Copies the translation."),
...
TC = [
    ...
    ("TC-22-1", "Tiles fill the phone screen", ["FR-22"], "System", "High", "Phone, portrait.",
     ["Open Home."],
     "The four tiles share the space between banner and language bar with no empty band below; each at least 140 dp tall."),
    ("TC-22-2", "Tablet sizes", ["FR-22", "NFR-08"], "System", "Medium", "Tablet (sw >= 600 dp).",
     ["Open Home, Result and PDF screens."],
     "Tiles >= 200 dp, primary buttons 72 dp, text 22 sp; nothing clipped."),
    ("TC-22-3", "Landscape / short screens scroll", ["FR-22"], "System", "Medium", "Phone, landscape.",
     ["Open Home.", "Scroll."], "Tiles keep their minimum height and the column scrolls; nothing overlaps."),
    ("TC-22-4", "Touch-target sizes", ["FR-22", "NFR-07"], "System", "Medium", "-",
     ["Run Accessibility Scanner on every screen."], "No touch target smaller than 48 dp is reported."),
...
KNOWN_ISSUES = [
    ...
    ("KI-06", "Build verification", "The UI changes (large controls, collapsible menu) and the rendering change have not yet "
